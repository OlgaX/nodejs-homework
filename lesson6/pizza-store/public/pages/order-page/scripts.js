...
const createdOrderStatusOutput = document.getElementById('createdOrderStatusOutput');
...
socket.on('changeOrderStatus', onChangeOrderStatus);

// on change order status

function onChangeOrderStatus(data) {
    if (data) {
        createdOrderStatusOutput.innerHTML = `Order #${data.orderId}<br>Status: <mark>${data.status}</mark>`;
    }
}
